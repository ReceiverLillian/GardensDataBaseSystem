...
<%@ page
import="bean.ConserveBean,dao.ConserveDao,bean.Converse_speciesBean,dao.Converse_speciesDao"%>
        				...
        				<div class="row">
        					<div class="col-lg-12">
        						<table id="data_list" class="table table-hover table-bordered"
        							...
        							<!---在此插入信息-->
        							<%
        								ArrayList<ConserveBean> allconverse = new ArrayList<ConserveBean>();
        								    ConserveDao condao=new ConserveDao();
        								    allconverse=(ArrayList<ConserveBean>)condao.getAllConserve();

        								for (ConserveBean bean : allconverse) {
        							...
        							<tbody>
        								...
        								<!-- 图书编号(PK)、图书名、出版时间、作者、出版社、图书分类、页数、价格、图书图片存放路径  -->
        								<td>
        								<button type="button" class="btn btn-warning btn-xs"
                                           onclick="javascrtpt:window.location.href='converse_detailed?bid=<%=bean.getCon_id()%>'">详情</button>
                                        </td>
        							</tbody>
        							<%
        								}
